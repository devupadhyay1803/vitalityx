...
export function DocumentCard({ document, isStaff, onDelete, onReplace }: DocumentCardProps) {
 ...
 {canPreview && (
 <button onClick={handlePreview} disabled={isLoading} className="btn btn-outline flex-1 sm:flex-none px-3 text-xs flex items-center justify-center gap-1.5" title="Preview">
 <Eye size={14} /> Preview
 </button>
 )}
 <button onClick={handleDownload} disabled={isLoading} className="btn btn-primary flex-1 sm:flex-none px-3 text-xs flex items-center justify-center gap-1.5" title="Download">
 <Download size={14} /> Download
 </button>
 
 {onReplace && (
 <button onClick={onReplace} disabled={isLoading} className="btn btn-outline px-3 text-xs flex items-center justify-center" title="Replace File">
 <RefreshCw size={14} />
 </button>
 )}
 {onDelete && (
 <button onClick={handleDelete} disabled={isLoading} className="btn btn-danger px-3 text-xs flex items-center justify-center" title="Delete">
 <Trash2 size={14} />
 </button>
 )}
 </div>
