...
export function NotificationsPopover({ variant }: { variant: "member" | "staff" }) {
 ...
 body: JSON.stringify({ action: "mark_all" }),
  ...
  const handleNotificationClick = (n: Notification) => {
    ...
    // Fallback for missing entity_id property in the type definition (though it exists in DB)
    const entityId = (n as any).entity_id;
    let target = n.link || getFallbackLink(n.type, variant);
    ...
    // Use window.location.href assignment as a fallback if router.push fails or seems unresponsive
    try {
      router.push(target);
    } catch (e) {
 ...
 {open && (
 <div className="absolute bottom-full left-0 mb-2 w-80 vx-card shadow-2xl z-50 overflow-hidden md:bottom-auto md:left-full md:top-0 md:ml-2 md:w-80">
